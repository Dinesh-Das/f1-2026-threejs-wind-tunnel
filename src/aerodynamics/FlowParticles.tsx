...
export function FlowParticles() {
  ...
  useFrame((state) => { if (material.current) { material.current.uniforms.uTime.value = state.clock.elapsedTime; material.current.uniforms.uSpeed.value = windSpeed / 350 } })
  return <points frustumCulled={false}>
    <bufferGeometry><bufferAttribute attach="attributes-position" args={[positions,3]} /></bufferGeometry>
    <shaderMaterial ref={material} transparent depthWrite={false} blending={THREE.AdditiveBlending} vertexShader={airflowVertex} fragmentShader={airflowFragment} uniforms={{ uTime:{value:0}, uSpeed:{value:windSpeed/350}, uColor:{value:new THREE.Color('#7de3ff')} }} />
  </points>
}
