...
export function Lighting() {
  ...
  return <>
    ...
    <directionalLight position={[4, 9, 5]} intensity={floorView ? .55 : aero ? 1.4 : 2.5} castShadow shadow-mapSize={[4096,4096]} shadow-bias={-.00008} shadow-normalBias={.025} />
    ...
    {!floorView && <Environment resolution={aero ? 128 : 256}>
      <Lightformer form="rect" intensity={aero ? 2 : 6} color="#ffffff" position={[0, 6, -2]} rotation={[Math.PI / 2, 0, 0]} scale={[10, 2, 1]} />
      <Lightformer form="rect" intensity={aero ? 1 : 4} color="#cfe7ff" position={[-5, 2, 2]} rotation={[0, Math.PI / 2, 0]} scale={[6, 1.3, 1]} />
      <Lightformer form="rect" intensity={aero ? 1.4 : 4.5} color="#ffffff" position={[5, 2, -2]} rotation={[0, -Math.PI / 2, 0]} scale={[5, 1, 1]} />
      <Lightformer form="ring" intensity={aero ? .8 : 2} color={team.primaryColor} position={[0, 2, -7]} scale={[2.5, 2.5, 1]} />
    </Environment>}
    {floorView && <>
