...
export function TopNav() {
  ...
  return (
    <header className="top-nav">
      <button
        className="brand"
        onClick={() => set({ cameraPreset: 'hero', selectedComponent: null, floorView: false, turntable: true })}
        aria-label="Reset to hero view"
      >
      ...
      <div className="nav-status">
        <span className="status-dot" /> {team.shortName} / {driver.shortName} <b>#{driver.number}</b>
      </div>
    </header>
  )
}
